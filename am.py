...
from telegram.ext import Application, CommandHandler, MessageHandler, filters, CallbackContext  # Change here
...
# Check if user has joined the channel
async def check_user_channel_join(update: Update, context: CallbackContext):
    chat_id = update.message.chat_id
    if check_channel_join(chat_id):
...
async def main():
    ...
    application = Application.builder().token(TELEGRAM_BOT_TOKEN).build()  # Change here
    ...
    application.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, check_user_channel_join))
